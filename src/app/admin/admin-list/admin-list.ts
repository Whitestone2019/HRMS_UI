...
export class AdminList implements OnInit, OnDestroy {
  files: any[] = [];
  editFileMap: any = {};
  selectedEditFileName: { [key: string]: string } = {};
  ...
  get isBlackScreenActive(): boolean {
    return AdminList.globalBlackScreenActive;
  }
  
  set isBlackScreenActive(value: boolean) {
    AdminList.globalBlackScreenActive = value;
  ...
  private loadBlackScreenState(): void {
    ...
    const savedState = localStorage.getItem('adminBlackScreenActive');
    if (savedState === 'true') {
      AdminList.globalBlackScreenActive = true;
      console.log('🖥️ Admin black screen state loaded from storage: ACTIVE');
    } else {
      AdminList.globalBlackScreenActive = false;
      console.log('🖥️ Admin black screen state loaded from storage: INACTIVE');
    }
  }

  private saveBlackScreenState(): void {
    // Save black screen state to localStorage
    localStorage.setItem('adminBlackScreenActive', String(AdminList.globalBlackScreenActive));
    console.log('💾 Admin black screen state saved to storage:', AdminList.globalBlackScreenActive);
  }
  ...
  public resetBlackScreenOnLogout(): void {
    console.log('🚪 LOGOUT DETECTED - Completely resetting admin black screen state');
    
    // Reset the global state
    AdminList.globalBlackScreenActive = false;
    
    // Remove from localStorage
    localStorage.removeItem('adminBlackScreenActive');
    ...
    console.log('✅ Admin black screen completely reset - application returned to normal state');
  }
  ...
  selectEditFile(event: any, fileObj: any): void {
    const file = event.target.files[0];
    if (file) {
      this.editFileMap[fileObj.fileName] = file;
      this.selectedEditFileName[fileObj.fileName] = file.name;
    }
  }

  edit(fileObj: any): void {
    const newFile = this.editFileMap[fileObj.fileName];
    if (!newFile) {
    ...
    this.apiService.editFile(fileObj.fileName, newFile).subscribe({
      next: () => {
        alert('File updated successfully');
        this.loadFiles();
        delete this.editFileMap[fileObj.fileName];
        delete this.selectedEditFileName[fileObj.fileName];
      },
      error: (err) => {
  ...
  delete(fileObj: any): void {
    ...
    this.apiService.deleteFile(fileObj.fileName).subscribe({
      next: () => {
  ...
  private applySimplifiedWatermark(canvas: HTMLCanvasElement): void {
    ...
    // Very subtle watermark - only 3% opacity
